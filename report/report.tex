...
\begin{itemize}
\item A stochastic number of daily clicks of new users (i.e., that have never clicked before on the ads), represented by the discrete random variable $N_{c,b}$, where $b \in B$ is a bid value. 
...
\subsection{The number of daily auctions $A$}
...
&=\sum_{c \in C}{ \lambda_a l_c v(b)\bigg(r(c,p)(1+f(c))m(p)-k(c,b)\bigg)}\\
...
\begin{align*}
ExpectedProfit(p,b)=\lambda_av(b)\sum_{c \in C}{l_{c}\Big(r(c,p)(1+f(c))m(p)-k(c,b)\Big)}\\
\end{align*}
\begin{align*}
